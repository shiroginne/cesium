...
module Cesium
  module Layout
    ...
    def cesium_layout_path controller = nil
      ...
      if @pages_cache.exists? self.path
        ...
        text = parser_init(controller).parse(self.get_layout.body)
    ...
    module ClassMethods
      ...
      def cesium_layout_path path, controller = nil
        path.gsub!(/\/$/, '')
        candidates = find(:all, :conditions => ['path like ?', path.scan(/^\/[\w-]*/)[0] + '%']).reverse
        for c in candidates do
          return c.cesium_layout_path(controller) if path =~ /^#{c.path.gsub(/\*/, '.*')}/
        end
